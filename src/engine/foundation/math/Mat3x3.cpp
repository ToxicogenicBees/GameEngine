/*
    Mat3x3.cpp

    Implementation of a 3x3 matrix.
*/

#include "foundation/math/Mat3x3.hpp"
#include "foundation/geometry/Size.hpp"
#include <stdexcept>
#include <cmath>

namespace toxico {
    Mat3x3::Mat3x3() noexcept
        : data_(Size{3, 3}, 0.0, GridOrder::ColumnMajor) {}

    Mat3x3 Mat3x3::identity() noexcept {
        Mat3x3 result;

        result(0, 0) = 1;
        result(1, 1) = 1;
        result(2, 2) = 1;

        return result;
    }

    Mat3x3 Mat3x3::translation(const Vector2& translation) noexcept {
        Mat3x3 result = Mat3x3::identity();

        result(0, 2) = translation.x;
        result(1, 2) = translation.y;

        return result;
    }

    Mat3x3 Mat3x3::rotation(fp_type rotation) noexcept {
        Mat3x3 result;
        auto sin = std::sin(rotation);
        auto cos = std::cos(rotation);

        result(0, 0) = cos;
        result(0, 1) = -sin;
        result(1, 0) = sin;
        result(1, 1) = cos;
        result(2, 2) = 1;

        return result;
    }

    Mat3x3 Mat3x3::scale(const Vector2& scale) noexcept {
        Mat3x3 result;

        result(0, 0) = scale.x;
        result(1, 1) = scale.y;
        result(2, 2) = 1;

        return result;
    }

    const fp_type& Mat3x3::operator()(std::size_t row, std::size_t col) const {
        return data_(row, col);
    }

    fp_type& Mat3x3::operator()(std::size_t row, std::size_t col) {
        return data_(row, col);
    }

    const fp_type& Mat3x3::at(std::size_t row, std::size_t col) const {
        try {
            return data_.at(row, col);
        }
        catch(...) {
            throw std::out_of_range("Accessed matrix out of range");
        }
    }

    fp_type& Mat3x3::at(std::size_t row, std::size_t col) {
        try {
            return data_.at(row, col);
        }
        catch(...) {
            throw std::out_of_range("Accessed matrix out of range");
        }
    }

    Mat3x3 Mat3x3::operator*(const Mat3x3& other) const {
        Mat3x3 result;

        for (std::size_t i = 0; i < Mat3x3::size; ++i) {
            for (std::size_t j = 0; j < Mat3x3::size; ++j) {
                fp_type dot = 0;
                for (std::size_t k = 0; k < Mat3x3::size; ++k)
                    dot += (*this)(i, k) * other(k, j);
                result(i, j) = dot;
            }
        }

        return result;
    }

    Mat3x3& Mat3x3::operator*=(const Mat3x3& other) {
        *this = *this * other;
        return *this;
    }
}
