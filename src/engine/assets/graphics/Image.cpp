...
namespace toxico {
    ...
    std::vector<Color4> Image::colors() const noexcept {
        // Store colors in a set while retrieving
        std::unordered_set<Color4> result;
        for (auto pixel : pixels_)
            result.insert(pixel);

        return std::vector<Color4>(result.begin(), result.end());
    }
